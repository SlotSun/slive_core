...
pub struct SliveDanmuConnection {
    inner: platforms_parser::danmaku::DanmuConnection,
}

impl SliveDanmuConnection {
...
macro_rules! impl_danmu_provider {
    ($name:ident, $inner:ty, $platform_id:expr) => {
        ...
        impl $name {
            ...
            ) -> anyhow::Result<SliveDanmuConnection> {
                let config =
                    platforms_parser::danmaku::ConnectionConfig::with_cookies(cookies);
                let conn = self.inner.connect(&room_id, config).await?;
                Ok(SliveDanmuConnection { inner: conn })
            }

            pub async fn receive(
                &self,
                connection: &SliveDanmuConnection,
            ) -> anyhow::Result<Option<types::SliveDanmuItem>> {
            ...
            pub async fn disconnect(
                &self,
                connection: &mut SliveDanmuConnection,
            ) -> anyhow::Result<()> {
...
impl SliveHuyaDanmakuProvider {
    ...
    pub async fn connect_with_extras(
        ...
        extras.insert("ayyuid".to_string(), ayyuid.to_string());
        extras.insert("top_sid".to_string(), top_sid.to_string());
        extras.insert("sub_sid".to_string(), sub_sid.to_string());
